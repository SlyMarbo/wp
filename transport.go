...
// A Transport is an HTTP/WP http.RoundTripper.
type Transport struct {
	...
	wpConns   map[string]Conn          // WP connections mapped to host:port.
	...
	// Priority is used to determine the request priority of WP
	// requests. If nil, wp.DefaultPriority is used.
...
func (t *Transport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if t.wpConns == nil {
		t.wpConns = make(map[string]Conn)
	}
	if t.tcpConns == nil {
	...
	if !ok || u.Scheme == "http" {
		...
		if tlsConn, ok := tcpConn.(*tls.Conn); ok {
			...
			case "wp/2":
				newConn, err := NewClientConn(tlsConn, t.PushReceiver)
				if err != nil {
					return nil, err
				}
				go newConn.Run()
				t.wpConns[u.Host] = newConn
				conn = newConn
			}
		} else {
...
// by setting wp.Transport.Receiver.
